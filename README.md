# 排版桌 IG Post Planner V8.4.4（流暢雙平台預覽版）
...
- 老闆預覽的 IG 圖面區不再攔截滑鼠滾輪，頁面可直接上下捲動；底部 Canva 左右翻頁仍可操作。
- 老闆預覽移除會隨捲動反覆重繪的點狀背景。
- 預覽圖片區加入繪製隔離與離屏內容略過，減少跨網域 iframe 的重繪範圍。
- 捲動期間暫停大面積卡片陰影，停止後立即恢復。
- 「圖文時間」的四張 Canva 預覽改為分批載入，避免五個 Canva 檢視器同時搶用主執行緒。
